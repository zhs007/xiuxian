...
- **Key Components:**
  ...
  - `CharacterManager`: Manages all active `Character` instances in the game. It provides methods to create, retrieve, and list characters, and includes special logic to track the single player character via a `getPlayer()` method. It will enforce a single-player rule by throwing an error if a second player character is created.
